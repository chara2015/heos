...
public final class FoliaLogFilterService {
    ...
    public static void installConfiguredFilters(Plugin p) {
        plugin = p;
        if (!plugin.getConfig().getBoolean("enableLogFilter", false) || installed) {
            return;
        }
        ...
        rootLogger.addFilter(new HeosLogFilter());
        context.updateLoggers();
        installed = true;
        plugin.getLogger().info("日志过滤: Enabled");
    }
    ...
    private static final class HeosLogFilter extends AbstractFilter {
        ...
        public Result filter(LogEvent event) {
            if (!plugin.getConfig().getBoolean("enableLogFilter", false)) {
            ...
            if (isUpdateSuppressionCrashNotice(message)) {
                ...
                plugin.getLogger().warning(FoliaMessages.updateSuppressionCrash(updateSuppressionDetail(message)));
                return Result.DENY;
            }
            if (matches(message, thrown)) {
                ...
                if (!recentCrashNotice()) {
                    plugin.getLogger().warning(FoliaMessages.updateSuppressionCrash(detail));
                }
                return Result.DENY;
            }
            return Result.NEUTRAL;
        ...
        private String updateSuppressionPosition(String message, Object[] parameters, Throwable thrown) {
            ...
            return FoliaMessages.unknownPosition();
        }
